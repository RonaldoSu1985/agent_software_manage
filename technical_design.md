...
### 1.1 架构概述
本系统采用前后端分离架构，前端使用 React + TypeScript，后端使用 FastAPI + SQLite。
...
├── init.sql                          # 数据库初始化脚本
└── agent_management.db               # SQLite数据库文件
```
...
│   ├── pages/
│   │   ├── LoginPage.tsx             # 登录页面
│   │   ├── InventoryList.tsx         # 库存列表页面
│   │   ├── PurchaseList.tsx          # 采购记录页面
│   │   ├── InstallList.tsx           # 安装记录页面
│   │   ├── TransferList.tsx          # 划拨记录页面
│   │   ├── StockLogs.tsx             # 库存变动记录页面
│   │   ├── UserList.tsx              # 用户管理页面
│   │   └── RoleList.tsx              # 角色管理页面
│   ├── App.tsx                       # 应用组件（路由配置）
│   ├── main.tsx                      # 应用入口
...
| system_type | String(20) | NotNull | 所属系统 |
| created_at | DateTime | Default=now() | 创建时间 |
